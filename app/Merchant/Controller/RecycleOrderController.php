...
class RecycleOrderController extends MerchantBaseController
{
    /**
    ...
    /***
     ** @api {get} merchant/getOrderList 获取回收订单列表
     ** @apiName 获取回收订单列表
     ** @apiGroup 回收订单
     ...
     * [0 => '待取货',1 => '待签收',2 => '待验收',3 => '待确认',4 => '寄回中',5 => '已完成',6 => '取消订单',7 => '部分取消',8 => '申请退回',9 => '已退回'];
    ...
    public function orderList(RequestInterface $request)
    {
        ...
        $orderList = ShopRecycleOrder::leftJoin('shop_order_express', 'shop_recycle_order.id', '=', 'shop_order_express.order_id')
            ...
            ->where('shop_recycle_order.merchant_id', $merchantId)
        ;
    ...
    /***
     ** @api {get} merchant/getOrderInfo 获取订单详细信息
     ** @apiName 获取订单详细信息
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
    ...
    public function orderInfo(RequestInterface $request)
    {
        ...
        $orderInfo = ShopRecycleOrder::where('id', $id)
            ->select('id','order_sn','source','order_type','member_id','member_real_name','price','recycle_price','order_status','created_at','merchant_id','merchant_logo','merchant_name','address','type','province','province_code','city','city_code','district','district_code','street','street_code','address','contact','phone','express_no','return_express_no')
            ->first();
    ...
    /***
     ** @api {get} merchant/closeOrder 取消订单
     ** @apiName 取消订单
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int} id          订单ID      必填
    ...
    {
        return $this->_updateOrder($request, 6, '取消订单');
    }


    /***
     ** @api {get} merchant/signFor 签收订单
     ** @apiName 签收订单
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int} id          订单ID      必填
    ...
    {
        return $this->_updateOrder($request, 2, '快递签收');
    }

    /***
     ** @api {get} merchant/finishOrder 完成验收
     ** @apiName 完成验收
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int} id          订单ID      必填
     ...
     ***/
    public function finishOrder(RequestInterface $request)
    {
        return $this->_updateOrder($request, 3, '完成验收');
    }

    /***
     ** @api {post} merchant/mailOrder 订单全部退回
     ** @apiName 订单全部退回
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int} id          订单ID      必填
    ...
    /***
     ** @api {get} merchant/getOrderRemark 获取订单备注
     ** @apiName 获取订单备注
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int} id          订单ID      必填
    ...
    /***
     ** @api {post} merchant/changeRemark 修改订单备注
     ** @apiName 修改订单备注
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int}    id          订单ID      必填
    ...
    /***
     ** @api {get} merchant/getOrderStatus 获取订单所以状态
     ** @apiName 获取订单所以状态
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     *
    ...
    /***
     ** @api {post} merchant/expressInformation 修改收件人信息
     ** @apiName 修改收件人信息
     ** @apiGroup 回收订单
     ** @apiHeader {string} token 已登录token(Header: token)  必填
     ** @apiParam {int}    id             订单ID      必填
...
//        }
    ...
    private function _updateOrder($request, $status, $name)
    {
        $info = $this->_checkId($request);
        if (!$info['status']) {
            return jsonError($info['msg'],400);
...
//                return jsonError('只有在待取货状态下才能取消订单',405);
//            }
//        }

        if ($name == '完成验收') {
            if ($orderInfo['order_status'] != 2) {
                return jsonError('只有在待验收状态下才能完成验收',405);
            }
        }


        $date['order_status'] = $status;
        $date['updated_at']   = time();
